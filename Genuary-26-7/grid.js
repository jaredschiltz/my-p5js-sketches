class Grid {
  ...
  // Bresenham's Line Algorithm
  draw_line_xor(start, stop, colour) {
    let x0 = start.x;
    let y0 = start.y;
    let x1 = stop.x;
    let y1 = stop.y;
    ...
    while (true) {
      let destination_pixel = this.get_pixel(createVector(x0, y0)).levels;
      let source_pixel = colour.levels;
      // perform xor
      if (
        ...
        this.set_pixel(createVector(x0, y0), color("#ffffff"));
      } else {
        this.set_pixel(createVector(x0, y0), colour); // Your function to light a pixel
      }
