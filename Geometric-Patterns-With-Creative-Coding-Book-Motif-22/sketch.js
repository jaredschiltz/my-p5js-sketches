...
class Motif {
  ...
  display() {
    ...
    b = (a * sin(108)) / sin(54);
    c = (a * sin(18)) / sin(54);
    d = (c * sin(72)) / sin(36); //Shape 1 - ten sided star
    let x, y;
    push(); //orient correctly
    rotate(18);
    beginShape();
    for (let i = 0; i < 10; i++) {
      x = a * cos(36 * i);
    ...
    endShape(CLOSE);
    pop(); //Shape 4 circular pattern with the u-type shape
    for (let i = 0; i < 10; i++) {
      push();
      rotate(i * 36);
...
function setup() {
  ...
  let g = e + a + f;
  dy = 2 * g * cos(36); //approximate the nRow and nCol values
