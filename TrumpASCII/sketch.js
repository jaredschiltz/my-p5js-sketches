...
let my_font;
const string =
  "BIGOT RACIST DISHONEST CLOWN DISGUSTING NARCISSIST BULLY MAN-CHILD STUPID EGOTISTICAL IGNORANT LIAR " +
...
function draw() {
  ...
  for (row = 0; row < img_height; row = row + cell_size) {
    for (col = 0; col < img_width; col = col + cell_size) {
      ...
      textAlign(LEFT);
      text(
        string[current_letter % string.length],
        col + text_horiz_offset,
        row + text_vert_offset
      );
      current_letter++;
    }
  }
