/*
Genuary 2026 - January 1th: One color, one shape
*/
...
function draw() {
  ...
  for (let row = 0; row < 9; row++) {
    ...
    for (let col = 0; col < col_number; col++) {
      ...
      draw_shape_two(foreground_color);
      pop();
    }
    for (let col = 0; col < col_number; col++) {
      push();
    ...
    for (let col = 0; col < col_number; col++) {
      push();
      translate(col * 94, 0);
      draw_shape_one(background_color);
      pop();
    }
    push();
    for (let col = 0; col < col_number; col++) {
